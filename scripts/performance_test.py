...
def monitor_poller(pid_file="poller.pid", duration_hours=24):
    ...
    logger.info(
        "Manual performance test: run poller in background, " "check memory every hour"
    )
    # For automated: find process by name
    process = None
    for proc in psutil.process_iter(["pid", "name"]):
        if proc.info["name"] == "python.exe" and "poller.py" in " ".join(
            proc.cmdline()
        ):
            process = proc
            break
    ...
    for i in range(duration_hours):
        ...
        logger.info(
            f"Hour {i+1}: RSS={mem_info.rss / 1024 / 1024:.2f} MB, " f"CPU={cpu}%"
        )
        time.sleep(3600)
